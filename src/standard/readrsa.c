...
/*   Updated: 2021/09/23 13:50:05 by aulopez          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int decode_public(t_sslrsa *arg, t_rsa *rsa)
{
	...
	if (ft_strncmp(sptr(arg->sin), "-----END PUBLIC KEY-----\n", 25) == 0)
		return (-1);
	if (decode_base64(&replacement, sptr(arg->sin) + 27, slen(arg->sin) - 52) == -1)
		return (-1);
	ret = 0;
...
int decode_public_rsa(t_sslrsa *arg, t_rsa *rsa)
{
	...
	if (ft_strncmp(sptr(arg->sin), "-----END RSA PUBLIC KEY-----\n", 29) == 0)
		return (-1);
	if (decode_base64(&replacement, sptr(arg->sin) + 31, slen(arg->sin) - 60) == -1)
		return (-1);
	ret = 0;
